...
using AAModClassic.___Content.Hoard.Projectiles;
    ...
    public class OreChunk : ModProjectile
    {
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            else if(k == ModContent.ItemType<CovetiteOre>())
            {
                for(int i = 0; i < 12; i++)
                {
                    NewProjectile(Projectile.position.X + 30f, Projectile.position.Y + 30f, Main.rand.Next(-3, 4), Main.rand.Next(-3, 10), ModContent.ProjectileType<GreedGold>(), Projectile.damage / 2, 1, Projectile.owner, 0, 1);
                }
            }
            else if(k == ModContent.ItemType<DarkmatterOre>())
            {
            ...
            else if(ModSupport.GetMod("CalamityMod") != null)
            {
                ...
                else if(k == ModSupport.GetModItem("CalamityMod", "PerennialOre").Item.type)
                {
                    ...
                    {
                        float x2 = Main.rand.NextBool(2) ? Projectile.Center.X + 100f : Projectile.Center.X - 100f;
