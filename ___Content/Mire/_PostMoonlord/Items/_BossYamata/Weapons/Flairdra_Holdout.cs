        ...
        public override void OnHitNPC (NPC target, NPC.HitInfo hit, int damageDone)
		{
            ...
            target.immune[Projectile.owner] = 1;
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 3f) * 5, (float)(Math.Cos(offsetAngle) * 3f) * 5, ModContent.ProjectileType<Projectiles.HydraSoulProj>(), Projectile.damage / 3, Projectile.knockBack, Projectile.owner, 0f, 0f);
                    Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 3f) * 5, (float)(-Math.Cos(offsetAngle) * 3f) * 5, ModContent.ProjectileType<Projectiles.HydraSoulProj>(), Projectile.damage / 3, Projectile.knockBack, Projectile.owner, 0f, 0f);
                }
