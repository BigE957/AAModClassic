        ...
        public override void AI()
        {
            ...
            if (MinionType == 21) //Stardust Summon
            {
                ...
                if (NPC.ai[3] > 400)
                {
                    ...
                    NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X + Xint, (int)NPC.Center.Y + Yint, ModContent.NPCType<OreConstruct>(), 0, Main.rand.Next(29));
                    for (int i = 0; i < 3; i++)
                    {
            ...
            if (properSide)
            {
                ...
                if (delayTimer <= 0)
                {
                    if (!checkCanHit || Collision.CanHit(codable.position, codable.width, codable.height, position, width, height))
                    {
                        ...
                        if (Main.projectile[pID].type == ModContent.ProjectileType<OreConstruct_Nebula>())
                        {
