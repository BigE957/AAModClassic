    ...
    public class ThunderLord_SuperchargedThunderbullet : ModProjectile
	{
        ...
        public void DropMeteor(NPC target)
        {
            ...
            for (int num113 = 0; num113 < num112; num113++)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), vector2.X, vector2.Y, num114 * 0.75f, num115 * 0.75f, ModContent.ProjectileType<ThunderLord_Thunderstrike>(), Projectile.damage, Projectile.damage, player.whoAmI, 0f, 0.5f + (float)Main.rand.NextDouble() * 2f);
            }
        }
    }
}