...
namespace AAModClassic._Content.Acropolis.__Hardmode.NPCs._BossAthena
{
    public class SeraphHerald : ModNPC
	{
        ...
        public override string Texture => "AAModClassic/_Content/Acropolis/__Hardmode/NPCs/_BossAthena/SeraphA";
        ...
        public override bool PreAI()
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                if (Vector2.Distance(player.Center, NPC.Center) > 2200)
                {
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        Dust d = Main.dust[Dust.NewDust(NPC.position, NPC.height, NPC.width, ModContent.DustType<FeatherDust>(), Main.rand.Next(-1, 2), 1, 0)];
                ...
                if (!NPCExtensions.BeenKilled<Greed>())
                {
                    if (NPC.ai[0] >= 480)
                    {
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            Dust.NewDust(NPC.position, NPC.height, NPC.width, ModContent.DustType<FeatherDust>(), Main.rand.Next(-1, 2), 1, 0);
                        }
