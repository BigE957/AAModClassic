...
namespace AAModClassic._Content.Acropolis.__Hardmode.NPCs.__BossAthena
{
    ...
    public class Athena : ModNPC
    {
                ...
                if (Vector2.Distance(NPC.Center, Acropolis) < 10)
                {
                    ...
                    if (!NPCExtensions.BeenKilled<Athena>())
                    {
                        if (WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial))
                        {
                            ...
                            {
                                case 30:
                                    int slot = DialogueDisplaySystem.GetSlot("Mods.AAModClassic.Athena.Intro.Multiplayer");
                                    if (slot != -1)
                    ...
                    else if (AAWorld.AthenaHerald && !NPCExtensions.BeenKilled<AthenaA>())
                    {
                        if (WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial))
                        {
                            ...
                            {
                                case 30:
                                    int slot = DialogueDisplaySystem.GetSlot("Mods.AAModClassic.Athena.Intro.PreOlympian");
                                    if (slot != -1)
                                        DialogueDisplaySystem.RemoveDialogue(slot);
                                    break;
                                case 60:
                    ...
                    else
                    {
                        if (WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial))
                        {
                            ...
                            {
                                case 30:
                                    int slot = DialogueDisplaySystem.GetSlot("Mods.AAModClassic.Athena.Intro.Repeat");
                                    if (slot != -1)
                                        DialogueDisplaySystem.RemoveDialogue(slot);
                                    break;
                                case 60:
        ...
        {
            Player player = Main.player[npc.target];
            int projType = ModContent.ProjectileType<Projectiles.SeraphFeather>();
        ...
        public override void OnKill()
        {
            if ((!WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial) && NPC.downedMoonlord) || (WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial) && NPCExtensions.BeenKilled<AnubisA>()))
            {
                ...
                {
                    int a = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X, (int)NPC.position.Y, ModContent.NPCType<AthenaA>());
                    Main.npc[a].Center = NPC.Center;
