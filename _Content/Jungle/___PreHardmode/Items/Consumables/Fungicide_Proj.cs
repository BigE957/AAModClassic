...
{
    public class Fungicide_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/Fungicide").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            int radius = 6;
            int FlaskDust = ModContent.DustType<Dusts.SwarmDust>();
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, ModContent.GetInstance<FungicideConversion>().Type, 1, true, true);
                    }
                }
            }
        }
