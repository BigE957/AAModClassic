...
namespace AAModClassic._Content.Inferno.__Hardmode.Items.Weapons
{
    public class DragonsPike : BaseAAItem, ILocalizedModType
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.shootSpeed = 10f;
            Item.shoot = ModContent.ProjectileType<DragonsPike_DragonHead>();
            Item.width = 64;
            Item.height = 64;
            ...
            Item.noMelee = true;
            Item.knockBack = 2f;
            Item.buffType = ModContent.BuffType<DragonsPike_Buff>();
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                int current = Projectile.NewProjectile(source, position.X, position.Y, 0, 0, ModContent.ProjectileType<DragonsPike_DragonHead>(), damage, knockback, player.whoAmI, 0f, 0f);

                int previous = 0;

                for (int i = 0; i < 1; i++)
                {
                    current = Projectile.NewProjectile(source, position.X, position.Y, 0, 0, ModContent.ProjectileType<DragonsPike_DragonBody>(), damage, knockback, player.whoAmI, current, 0f);
                    previous = current;
                }

                current = Projectile.NewProjectile(source, position.X, position.Y, 0, 0, ModContent.ProjectileType<DragonsPike_DragonTail>(), damage, knockback, player.whoAmI, current, 0f);
            ...
            else
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    current = Projectile.NewProjectile(source, position, velocity, ModContent.ProjectileType<DragonsPike_DragonBody>(), damage, knockback, player.whoAmI,
