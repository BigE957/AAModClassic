...
{
    public class MorningGlory_Proj : ModProjectile
    {
        ...
        {	
			...
			if (Projectile.ai[0] == 1f || Projectile.ai[0] == 3f)
			{
				...
				for (int i = 0; i < numberProjectiles; i++)
				{
					...
					int proj = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<MorningGlory_Proj>(),  Projectile.damage, Projectile.knockBack, Projectile.owner, 0, -1f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
			...
			if (Projectile.ai[1] == -1f) texture = ModContent.Request<Texture2D>(Texture + "_Phantom").Value;
        ...
        public override void OnKill(int i)
        {
			...
			Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.position, Projectile.velocity, ModContent.ProjectileType<AkumaExplosionBlue>(), Projectile.damage, Projectile.knockBack, Projectile.owner, Projectile.whoAmI);
        }
	}
}
