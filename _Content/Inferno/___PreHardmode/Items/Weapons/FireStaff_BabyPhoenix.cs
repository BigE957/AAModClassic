...
namespace AAModClassic._Content.Inferno.___PreHardmode.Items.Weapons
{
    public class FireStaff_BabyPhoenix : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                if (p.whoAmI != Projectile.whoAmI && p.owner == Projectile.owner && p.type == Projectile.type && Math.Abs(Projectile.position.X - p.position.X) + Math.Abs(Projectile.position.Y - p.position.Y) < Projectile.width)
                {
