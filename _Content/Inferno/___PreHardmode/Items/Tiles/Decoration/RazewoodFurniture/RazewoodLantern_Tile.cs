...
namespace AAModClassic._Content.Inferno.___PreHardmode.Items.Tiles.Decoration.RazewoodFurniture
{
    public class RazewoodLantern_Tile : ModTile
	{
        ...
        public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
        {
            ...
            for (int k = 0; k < 7; k++)
            {
                Main.spriteBatch.Draw(ModContent.Request<Texture2D>("AAModClassic/_Content/Inferno/___PreHardmode/Items/Tiles/Decoration/RazewoodFurniture/RazewoodLantern_Tile_Glow").Value, new Vector2(i * 16 - (int)Main.screenPosition.X + offsetX - (width - 16f) / 2f, j * 16 - (int)Main.screenPosition.Y + offsetY) + zero, new Rectangle(frameX, frameY, width, height), color, 0f, default, 1f, SpriteEffects.None, 0f);
            }
        }
    }
}
