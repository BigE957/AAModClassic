			...
			for (int i = 0; i < 200; i++)
            {
                ...
                if (distance < 600f && target.catchItem == 0 && !target.friendly && target.active && target.type != NPCID.TargetDummy)
                {
                    if (Projectile.ai[0] > 30f) // Time in (60 = 1 second) 
                    {
                        ...
                        shootToX *= distance * 3;
                        shootToY *= distance * 3;
                        int id = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, shootToX*4, shootToY*4, ModContent.ProjectileType<ForsakenStaff_ForsakenFrag>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, 0f, 0f);
