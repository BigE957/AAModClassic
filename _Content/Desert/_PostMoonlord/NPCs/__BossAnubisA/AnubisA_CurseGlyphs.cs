...
namespace AAModClassic._Content.Desert._PostMoonlord.NPCs.__BossAnubisA
{
	...
	{				
		...
		public override void AI()
        {
            if (body == -1)
            {
                int npcID = BaseAI.GetNPC(Projectile.Center, ModContent.NPCType<AnubisA>(), 400f, null);
                if (npcID >= 0) body = npcID;
            }
            if (body == -1) return;
            NPC anubis = Main.npc[body];
            if (anubis == null || anubis.life <= 0 || !anubis.active || anubis.type != ModContent.NPCType<AnubisA>()) { Projectile.active = false; return; }

            Projectile.rotation += .1f;

            int glyph = ((AnubisA)anubis.ModNPC).RuneCount;
