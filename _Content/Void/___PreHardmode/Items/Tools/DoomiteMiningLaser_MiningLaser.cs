...
namespace AAModClassic._Content.Void.___PreHardmode.Items.Tools
{
    ...
    public class DoomiteMiningLaser_MiningLaser : ModProjectile
	{
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(ModContent.Request<Texture2D>(Texture + "_Glow").Value, vector46 - Main.screenPosition + value50 * num298, null, new Color(255, 255, 255, 0), Projectile.rotation + 1.57079637f + (spriteEffects == SpriteEffects.None ? 3.14159274f : 0f), new Vector2(spriteEffects == SpriteEffects.None ? 0 : texture2D37.Width, texture2D37.Height / 2f) + Vector2.UnitY * 1f, Main.player[Projectile.owner].inventory[Main.player[Projectile.owner].selectedItem].scale, spriteEffects, 0f);
            if (num296 > num299)
            {
