...
{
    public class VoidFlask_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/VoidFlask").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            int radius = 6;
            int FlaskDust = ModContent.DustType<Dusts.VoidDust>();
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, ModContent.GetInstance<VoidConversion>().Type, 1, true, true);
                    }
                }
            }
        }
