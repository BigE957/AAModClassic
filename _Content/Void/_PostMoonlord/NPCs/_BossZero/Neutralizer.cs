...
namespace AAModClassic._Content.Void._PostMoonlord.NPCs._BossZero
{
    ...
    public class Neutralizer: ModNPC
    {
        ...
        {
            NPC.TargetClosest(); NPC.TargetClosest();
            if (body == -1)
            {
            ...
            if (NPC.ai[2] >= aiTimerFire)
            {
                ...
                for (int i = 0; i < Arrows; i++)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ModContent.ProjectileType<ZArrow>(), NPC.damage / 2, 5, Main.myPlayer);
                }
                NPC.netUpdate = true;
                NPC.ai[2] = 0;
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Texture2D glowTex = ModContent.Request<Texture2D>("AAModClassic/Glowmasks/Neutralizer2_Glow").Value;
