...
{
    public class ZeroA_GlitchRocket : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
	    	...
	    	if (Projectile.owner == Main.myPlayer)
	    	{
		    	for (i = 0; i < 4; i++ )
		    	{
		        	...
		        	Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center.X, Projectile.Center.Y, (float)( Math.Sin(offsetAngle) * 2f ), (float)( Math.Cos(offsetAngle) * 6f ), ModContent.ProjectileType<ZeroA_GlitchBlast>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
		        	Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center.X, Projectile.Center.Y, (float)( -Math.Sin(offsetAngle) * 2f ), (float)( -Math.Cos(offsetAngle) * 6f ), ModContent.ProjectileType<ZeroA_GlitchBlast>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
		    	}
	    	}
        }
