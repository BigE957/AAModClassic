...
{
    [AutoloadBossHead]
    public class ZeroRealityCannon : ModNPC
    {
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry)
        {
            ...
            [
                new ColoredFlavorTextBestiaryInfoElement("Mods.AAModClassic.Bestiary.ZeroRealityCannon", AAColor.OblivionDialogue)
            ]);
        }
            ...
            if (NPC.ai[2] == aiTimerFire)
            {
                ...
                if (Collision.CanHit(NPC.position, NPC.width, NPC.height, player.Center, player.width, player.height))
                {
                    ...
                    for (int i = 0; i < (Main.expertMode ? 3 : 4); i++)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ModContent.ProjectileType<DeathLaser>(), 41, 5, -1);
                    }
                }
            }
