...
namespace AAModClassic._Content.Void._PostMoonlord.NPCs.__BossZero
{
    ...
    class ZeroRiftShredder_Rift : ModProjectile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            Triangle1 = ModContent.Request<Texture2D>(Texture + "1");
            Triangle2 = ModContent.Request<Texture2D>(Texture + "2");
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D Tex2 = Triangle1.Value;
            Texture2D Tex3 = Triangle2.Value;
        ...
        public override void OnHitPlayer(Player target, Player.HurtInfo info)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 5f), (float)(Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<ZeroRiftShredder_RiftSlash>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, 0f, 0f);
                Projectile.NewProjectile(Projectile.GetSource_OnHit(target), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 5f), (float)(-Math.Cos(offsetAngle) * 5f), ModContent.ProjectileType<ZeroRiftShredder_RiftSlash>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, 0f, 0f);
            }
            for (int k = 0; k < 10; k++)
            {
