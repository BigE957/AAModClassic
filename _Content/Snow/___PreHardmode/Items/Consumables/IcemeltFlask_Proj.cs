...
{
    public class IcemeltFlask_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/IcemeltFlask").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            int radius = 6;
            int FlaskDust = DustID.GoldFlame;
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, ModContent.GetInstance<TundraRemovalConversion>().Type, 1, true, true);
                    }
                }
            }
        }
