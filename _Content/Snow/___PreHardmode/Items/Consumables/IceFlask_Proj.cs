...
{
    public class IceFlask_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/IceFlask").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, ModContent.GetInstance<TundraConversion>().Type, 1, true, true);
                    }
                }
            }
        }
