...
namespace AAModClassic._Content.Corruption.___PreHardmode.Items.Consumables
{
    public class CorruptionFlask_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/CorruptionFlask").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, 1, 1, true, true);
                    }
                }
            }
        }
