...
{
    public class OrderBottle_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/OrderBottle").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            int radius = 6;
            int FlaskDust = ModContent.DustType<Dusts.OrderSolution>();
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, ModContent.GetInstance<ChaosRemovalConversion>().Type, 1, true, true);
                    }
                }
            }
        }
