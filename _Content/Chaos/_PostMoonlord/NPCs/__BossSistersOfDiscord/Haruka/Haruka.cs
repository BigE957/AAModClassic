...
using static AAModClassic._Content.Chaos._PostMoonlord.NPCs.__BossSistersOfDiscord.Ashe.Ashe;
    ...
    public class Haruka : ModNPC
    {
        ...
        public override void SendExtraAI(BinaryWriter writer)
        {
            ...
            if (Main.netMode == NetmodeID.Server || Main.dedServ)
            {
                ...
                writer.Write(internalAI[9]); //Noticing Ashe is dead
        ...
        public override void ReceiveExtraAI(BinaryReader reader)
        {
            ...
            if (Main.netMode == NetmodeID.MultiplayerClient)
            {
                ...
                internalAI[8] = reader.ReadInt32();
                internalAI[9] = reader.ReadInt32();
        ...
        public override void OnKill()
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient) BaseUtility.Chat(Language.GetTextValue("Mods.AAModClassic.NPCs.BossDialogue.SistersOfDiscord.HarukaDowned"), new Color(72, 78, 117));
        ...
        public static int AISTATE_PROJ = 0, AISTATE_SLASH = 1, AISTATE_SPIN = 2, AISTATE_IDLE = 3, AISTATE_Shadowkilling = 4, AISTATE_ASHEGAG = 5;
        ...
        public override void AI()
        {
            ...
            if (ashe == 0)
            {
                ...
                if (flag)
                {
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        int DeathAnim = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<HarukaVanish>(), 0);
            ...
            if (internalAI[5] > InvisTimer2 && Main.netMode != NetmodeID.MultiplayerClient && internalAI[0] != AISTATE_Shadowkilling)
            {
                Invisible = false;
                Backstab();
        ...
        public override void FindFrame(int frameHeight)
        {
            ...
            if (internalAI[0] == AISTATE_IDLE || internalAI[0] == AISTATE_ASHEGAG)
            {
                if (Frame > 3)
                {
                    NPC.frameCounter = 0;
        ...
        public void Shadowkilling()
        {
            ...
            if(NPC.alpha >= 255 && SpawnClone)
            {
                ...
                if(Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    ShadowNPC[2] = NPC.NewNPC(NPC.GetSource_FromThis(), (int)spawnpoint[k2].X, (int)spawnpoint[k2].Y, ModContent.NPCType<HarukaClone>(), 0, NPC.whoAmI);
            ...
            if(internalAI[4] >= 160 || Main.npc[ShadowNPC[1]].Hitbox.Intersects(Main.npc[ShadowNPC[0]].Hitbox) || Main.npc[ShadowNPC[1]].Hitbox.Intersects(Main.npc[ShadowNPC[2]].Hitbox) || Main.npc[ShadowNPC[2]].Hitbox.Intersects(Main.npc[ShadowNPC[0]].Hitbox))
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), ShadowkingPosition.X, ShadowkingPosition.Y, 0, 0, ModContent.ProjectileType<Haruka_DepthKillingBlast>(), 45, 5);
                    int projType = ModContent.ProjectileType<Haruka_NightSlash>();
                    for (int i = 0; i < 12; i++)
                    {
                        shoot = new Vector2((float)Math.Sin(i * (Pi / 6)), (float)Math.Cos(i * (Pi / 6)));
                        shoot *= 10f;
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), ShadowkingPosition.X, ShadowkingPosition.Y, shoot.X, shoot.Y, projType, 18, 5);
                    }
                }
