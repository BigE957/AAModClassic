...
{
    public class FlameVortex : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (body == -1)
            {
                int npcID = BaseAI.GetNPC(NPC.Center, ModContent.NPCType<Ashe>(), 120f, null);
                if (npcID >= 0) body = npcID;
            }
            if (body == -1) return;

            NPC ashe = Main.npc[body];
            if (ashe == null || ashe.life <= 0 || !ashe.active || ashe.type != ModContent.NPCType<Ashe>()) { NPC.active = false; return; }
        ...
        public override void OnKill()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                ...
                Projectile.NewProjectile(NPC.GetSource_Death(), NPC.Center.X, NPC.Center.Y, (float)(Math.Sin(offsetAngle) * 7f), (float)(Math.Cos(offsetAngle) * 7f), ModContent.ProjectileType<FlameVortex_FireBomb>(), NPC.damage / 2, 0, Main.myPlayer, 0f, 0f);
            }
        }
