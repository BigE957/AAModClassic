...
namespace AAModClassic._Content.Chaos._PostMoonlord.NPCs.__BossSistersOfDiscord.Ashe
{
    ...
    public class Ashe : ModNPC
    {
        ...
        public static Asset<Texture2D> Ring1;
        public static Asset<Texture2D> Ring2;
        public static Asset<Texture2D> Ritual;
        ...
        public override void AI()
        {
            ...
            switch (NPC.ai[0])
            {
                ...
                case 7: //dashing
                    if (++NPC.ai[2] > 3)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(Math.PI / 2), ModContent.ProjectileType<Ashe_Fireball>(), NPC.damage / 4, 0f, Main.myPlayer, ai0);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(-Math.PI / 2), ModContent.ProjectileType<Ashe_Fireball>(), NPC.damage / 4, 0f, Main.myPlayer, ai0);
                        }
                    }
                    if (++NPC.ai[1] > 40)
                        ...
                        if (NPC.life < NPC.lifeMax / 3)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                for(int i = 0; i < 8; i++)
                                {
                                    ...
                                    shoot *= 8f;
                                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, shoot.X, shoot.Y, ModContent.ProjectileType<Ashe_FireBomb>(), NPC.damage / 4, 5, Main.myPlayer, 0f, 0f);
                                }
                            }
                ...
                case 10:
                    if (!AliveCheck(player))
                        break;
                    if (NPC.AnyNPCs(ModContent.NPCType<AshenDragonHead>()))
                    {
                        NPC.ai[0] = 12;
                        NPC.netUpdate = true;
                        goto case 12;
                ...
                case 11:
                    ...
                    if (NPC.ai[1]++ > 200)
                    {
                        int id = NPC.NewNPC(NPC.GetSource_FromThis(), (int)NPC.position.X, (int)NPC.position.Y, ModContent.NPCType<AshenDragonHead>());
        ...
        public void FireMagic(NPC npc)
        {
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (Health && npc.life >= npc.lifeMax * .66f)
                {
                    ...
                    for (int m = 0; m < 4; m++)
                    {
                        int n = NPC.NewNPC(NPC.GetSource_FromThis(), (int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<FlameVortex>(), 0, npc.whoAmI, distance, 300, rotation * m);
        ...
        private void RingEffects()
        {
            RingRotation += 0.02f;
            RingRotation2 -= 0.02f;
            if (NPC.ai[0] == 12 || NPC.AnyNPCs(ModContent.NPCType<FlameVortex>()))
            {
                if (scale >= 1f)
                {
                    scale = 1f;
