...
namespace AAModClassic._Content.Chaos._PostMoonlord.NPCs.__BossShenDoragon
{
    ...
    public class ShenDoragon : ModNPC
    {
        ...
        public void NewProjectileFlipped<T>(IEntitySource spawnSource, Vector2 position, Vector2 velocity, int Damage, float KnockBack, int Owner = 1, float ai0 = 0, float ai1 = 0, float ai2 = 0) where T : ShenDoragon_ChaosFireballAbstract
        {
            ...
            if (NPC.type == ModContent.NPCType<ShenDoragonA>())
            ...
            switch ((int)NPC.ai[0])
            {
                case 0: //target for first time, navigate beside player (spawn deathray if awakened)
                    ...
                    if (++NPC.ai[1] > 60)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            for (int i = -2; i <= 2; i++)
                                NewProjectileFlipped<ShenDoragon_ChaosFireballSpread>(NPC.GetSource_FromThis(), NPC.Center, 30 * Vector2.UnitX.RotatedBy(Math.PI / 4 * i) * (NPC.Center.X < player.Center.X ? -1 : 1), NPC.damage / 4, 0f, Main.myPlayer, 20, 20 + 60);
                    }
                    break;
                ...
                case 6: //fly at player, spit mega balls
                    ...
                    if (++NPC.ai[2] > 80)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            NewProjectileFlipped<ShenDoragon_ChaosFireballFrag>(NPC.GetSource_FromThis(), spawnPos, vel, NPC.damage / 4, 0f, Main.myPlayer);
                        }
                    }
                    if (++NPC.ai[1] > 210)
                    {
                ...
                case 10: //dashing
                    if (++NPC.ai[2] > 3)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            NewProjectileFlipped<ShenDoragon_ChaosFireballAccel>(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(Math.PI / 2), NPC.damage / 4, 0f, Main.myPlayer, ai0);
                            NewProjectileFlipped<ShenDoragon_ChaosFireballAccel>(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity).RotatedBy(-Math.PI / 2), NPC.damage / 4, 0f, Main.myPlayer, ai0);
                        }
                    }
                ...
                case 11: //fly up, prepare to spit mega homing and dash
                    ...
                    if (++NPC.ai[1] > 180 || NPC.Distance(targetPos) < 50)
                    {
                        ...
                        NPC.velocity.Y = 5f;
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                            NewProjectileFlipped<ShenDoragon_ChaosFireballHoming>(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, NPC.damage / 3, 0f, Main.myPlayer, NPC.target, 8f);
                    }
                    NPC.rotation = 0;
                    break;
                    ...
                    if (++NPC.ai[2] > 60)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient) //spawn lightning
                        {
                            ...
                            else
                            {
                                ...
                                int projectile = Projectile.NewProjectile(NPC.GetSource_FromThis(), infernoPos, vel, ModContent.ProjectileType<DiscordianInferno>(), damage, 0f, Main.myPlayer, InfernoType, 0f);
        ...
        public override void OnKill()
        {
            if (!IsAwakened)
            {
                if (!Main.expertMode)
                {
                    ...
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient) ChatUtils.Chat(Language.GetTextValue("Mods.AAModClassic.NPCs.BossDialogue.ShenDoragon.Defeat.NotExpert.First"), Color.DarkMagenta.R, Color.DarkMagenta.G, Color.DarkMagenta.B);
                    ...
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient) ChatUtils.Chat(Language.GetTextValue("Mods.AAModClassic.NPCs.BossDialogue.ShenDoragon.Defeat.NotExpert.Repeat"), Color.DarkMagenta.R, Color.DarkMagenta.G, Color.DarkMagenta.B);
                    };
        ...
        {
<<<<<<< HEAD:_Content/Chaos/_PostMoonlord/NPCs/__BossShenDoragon/Shen.cs
            Texture2D currentTex = NPC.spriteDirection == 1 ? ModContent.Request<Texture2D>("AAModClassic/_Content/Chaos/_PostMoonlord/NPCs/__BossShenDoragon/ShenDoragonBlue").Value : TextureAssets.Npc[NPC.type].Value;
            Texture2D currentWingTex = NPC.spriteDirection == 1 ? ModContent.Request<Texture2D>("AAModClassic/_Content/Chaos/_PostMoonlord/NPCs/__BossShenDoragon/ShenDoragonBlueWings").Value : ModContent.Request<Texture2D>("AAModClassic/_Content/Chaos/_PostMoonlord/NPCs/__BossShenDoragon/ShenDoragonWings").Value;

=======
>>>>>>> 670fbbe856f0c57fb0bd4e683d47971012388a1b:_Content/Chaos/_PostMoonlord/NPCs/__BossShenDoragon/ShenDoragon.cs
            //offset
            NPC.position.Y += 130f;
