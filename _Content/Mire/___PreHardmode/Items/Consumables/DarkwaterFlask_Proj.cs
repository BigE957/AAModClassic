...
{
    public class DarkwaterFlask_Proj : ModProjectile
    {
        ...
        {
            Texture2D tex = ModContent.Request<Texture2D>("AAModClassic/Projectiles/Flasks/DarkwaterFlask").Value;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int x = -radius; x <= radius; x++)
            {
                for (int y = -radius; y <= radius; y++)
                {
                    ...
                    {
                        WorldGen.Convert(xPosition, yPosition, ModContent.GetInstance<MireConversion>().Type);
                    }
                }
            }
        }
