...
{
    public class MobianBuster_Holdout : ModProjectile
    {
        ...
        public override void AI()
        {
			...
			if (Projectile.ai[1] == 1f && Projectile.ai[0] != 1f)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					int num5 = Dust.NewDust(value - Vector2.One * 8f, 16, 16, DustID.Pixie, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f, 100);
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                switch (chargeLevel)
                {
                    case 0:
						...
						Projectile.NewProjectile(Projectile.GetSource_Death(), vector2.X, vector2.Y, velocity.X, velocity.Y, ModContent.ProjectileType<MobianBuster_MobianShot>(), Projectile.damage, 1f, player.whoAmI);
						break;
					case 1:
						SoundEngine.PlaySound(SoundID.Item89, Projectile.position);
                        Projectile.NewProjectile(Projectile.GetSource_Death(), vector2.X, vector2.Y, velocity.X, velocity.Y, ModContent.ProjectileType<MobianBuster_MobianShot>(), Projectile.damage*2, 1f, player.whoAmI);
						break;
					case 2:
					...
					case 3:
						SoundEngine.PlaySound(SoundID.Item88, Projectile.position);
                        Projectile.NewProjectile(Projectile.GetSource_Death(), vector2.X, vector2.Y, velocity.X, velocity.Y, ModContent.ProjectileType<MobianBuster_MobianChargedShot>(), Projectile.damage*8, 1f, player.whoAmI);
						break;
                }
            }
        }
