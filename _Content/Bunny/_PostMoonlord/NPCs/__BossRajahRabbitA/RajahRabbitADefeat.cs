    ...
    public class RajahRabbitADefeat : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.ai[0] == 1660)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    if (Main.netMode == NetmodeID.SinglePlayer)
                        BaseUtility.Chat(Language.GetOrRegister("Mods.AAModClassic.NPCs.BossDialogue.Rajah.Awakened.Defeat.First.11.Singleplayer").Format(Main.LocalPlayer.name), 107, 137, 179, true);
                    else
            ...
            if (NPC.ai[0] >= 2180)
            {
                ...
                int p = Projectile.NewProjectile(NPC.GetSource_Death(), NPC.position, NPC.velocity, ModContent.ProjectileType<RajahRabbitALeave>(), 0, 0, Main.myPlayer);
