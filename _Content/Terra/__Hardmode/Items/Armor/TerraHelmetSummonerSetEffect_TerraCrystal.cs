    ...
    public class TerraHelmetSummonerSetEffect_TerraCrystal : ModProjectile
	{
        ...
        public override void AI()
		{
            ...
            if (modPlayer.CrystalMode != 1)
			{
                ...
                if (WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial))
                {
                    Projectile.rotation = Projectile.velocity.X * 0.04f * player.gravDir;
                    if (player.gravDir == -1f)
                        Projectile.rotation += (float)Math.PI;
                }
			...
			else
			{
                ...
                {
                    bool flag23 = p.type == ModContent.ProjectileType<TerraHelmetSummonerSetEffect_TerraCrystal>();
                ...
                if (Projectile.ai[0] == 0f)
                {
                    ...
                    if (flag25 && Projectile.ai[1] == 0f)
                    {
                        ...
                        if (Main.myPlayer == Projectile.owner && Collision.CanHitLine(Projectile.position, Projectile.width, Projectile.height, vector46, 0, 0))
                        {
                            ...
                            Main.projectile[num659].DamageType = WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial) == true ? DamageClass.Summon : DamageClass.Melee;
                            Projectile.netUpdate = true;
                        }
                    }
                }
        ...
        public void MoveToPoint(Vector2 point)
		{
            ...
            if (WorldTypeSystem.IsWorldOptionEnabled(AAWorldOption.Unofficial))
            {
                ...
                Projectile.velocity *= MathHelper.Lerp(0f, 1f, length / moveSpeed) * 2f;
            }
            else
            {
                if (length < moveSpeed)
