...
{
    public class DarkmatterHelmetSummonerSetEffect : EquipmentEffectData
    {
        public override void DoEffect(Player player)
        {
            player.GetModPlayer<DarkmatterHelmetSummonerSetPlayer>().setBonus = true;
        }
    }

    public class DarkmatterHelmetSummonerSetPlayer : ModPlayer
    {
    ...
    public class DarkmatterHelmetSummonerSetProjectile : GlobalProjectile
    {
        //power settings
        ...
        public override bool PreAI(Projectile projectile)
        {
            ...
            if (projectile.minion && projectile.minionSlots > 0 && projectile.active && Main.player[projectile.owner].GetModPlayer<DarkmatterHelmetSummonerSetPlayer>().setBonus && cooldown == 0)
            {
        ...
        void DarkBlast(Projectile projectile)
        {
            ...
            for (int p = 0; p < Main.projectile.Length; p++)
            {
                ...
                {
                    Main.projectile[p].GetGlobalProjectile<DarkmatterHelmetSummonerSetProjectile>().reduceDamage += damageReductionPerBlast;
                }
            }
        }
        ...
        {
            if (projectile.GetGlobalProjectile<DarkmatterHelmetSummonerSetProjectile>().reduceDamage > 0)
            {
                float v = projectile.GetGlobalProjectile<DarkmatterHelmetSummonerSetProjectile>().reduceDamage / (float)EstimatedDamage(projectile);
