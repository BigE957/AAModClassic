...
namespace AAMod.NPCs.Bosses.Equinox
{
    public class DayBringerBlast : ModProjectile
    {
        ...
        {
            // DisplayName.SetDefault("Day Ray");
        }
        ...
        private void DrawLaser(SpriteBatch spritebatch, Vector2 centre)
        {
            ...
            if (Projectile.ai[0] > 0)
            {
                ...
                Utils.DrawLaser(
                    spritebatch,
                    Mod.GetTexture("NPCs/Bosses/Equinox/DayBringerBlast_Beam"),
                    start - Main.screenPosition,
                    end - Main.screenPosition,
                    new Vector2(Projectile.ai[1]),
                    new Utils.LaserLineFraming(DayBringerLaser)); //uses delegate (see method below)
            }
            
            
        }
        //define which frames are used in each stage (0 = start, 1 = mid, 2 = end
        private void DayBringerLaser(int stage, Vector2 currentPosition, float distanceLeft, Rectangle lastFrame, out float distCovered, out Rectangle frame, out Vector2 origin, out Color color)
        {
            color = new Color(250, 244, 171);
			if (stage == 0)
			{
				distCovered = 33f;
