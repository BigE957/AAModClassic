        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, vel, ModContent.ProjectileType<ShenFireballAccel>(), Projectile.damage, 0f, Main.myPlayer, Math.Abs(ai), ai);
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, vel, ModContent.ProjectileType<ShenFireballAccel>(), Projectile.damage, 0f, Main.myPlayer, Math.Abs(ai), -ai);
                }
            }
        }
