        ...
        public override void AI()
        {
        	...
        	if(Main.player[Projectile.owner].itemAnimation < Main.player[Projectile.owner].itemAnimationMax / 3)
        	{
                ...
                if (Projectile.localAI[0] == 0f && Main.myPlayer == Projectile.owner && !AAGlobalProjectile.AnyProjectiles(Mod.Find<ModProjectile>("ChaosYariEXShot").Type))
                {
                    ...
                    Projectile.NewProjectile(Projectile.position.X, Projectile.position.Y, Projectile.velocity.X * 1.4f, Projectile.velocity.Y * 1.4f, Mod.Find<ModProjectile>("ChaosYariEXShot").Type, (int)((double)Projectile.damage * 0.85f), Projectile.knockBack * 0.85f, Projectile.owner, 0f, 0f);
                    Projectile.NewProjectile(Main.player[Projectile.owner].position.X, Main.player[Projectile.owner].position.Y, Projectile.velocity.X * 1.3f, Projectile.velocity.Y * 1.3f, Mod.Find<ModProjectile>("ChaosYariEXShot").Type, (int)((double)Projectile.damage * 0.85f), Projectile.knockBack * 0.85f, Projectile.owner, 0f, 0f);
				}
        	}
        	else
        	{
