    ...
    public class TimeTeller : ModProjectile   //make sure the sprite file is named like the class name (CustomYoyoProjectile)
    {
        ...
        public override void PostAI()
        {
            ...
            if (Projectile.localAI[1] >= 25f)
            {
                ...
                vector *= 0.8f;
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X - vector.X, Projectile.Center.Y - vector.Y, vector.X, vector.Y, ModContent.ProjectileType<Time>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
