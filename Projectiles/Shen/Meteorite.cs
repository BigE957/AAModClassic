...
namespace AAModClassic.Projectiles.Shen
{
    public class Meteorite : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (TileHit)
            {
                int proj = Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center.X, Projectile.Center.Y + 20, Projectile.velocity.X, Projectile.velocity.Y, Mod.Find<ModProjectile>("MeteorStrike").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                Main.projectile[proj].DamageType = DamageClass.Melee;
            }
            if (EnemyHit)
            {
                int proj = Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.Center.X, Projectile.Center.Y + 20, Projectile.velocity.X, Projectile.velocity.Y, Mod.Find<ModProjectile>("MeteorBoom").Type, Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
                Main.projectile[proj].DamageType = DamageClass.Melee;
            }
        }
