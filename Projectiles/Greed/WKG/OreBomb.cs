    ...
    public class OreBomb : ModProjectile
    {
        ...
        public override void OnKill(int a)
        {
            ...
            for (int i = 0; i < Main.rand.Next(5, 10); i++)
            {
                ...
                int p = Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.position, new Vector2(x, y), ModContent.ProjectileType<OreChunkM>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, 0, Main.rand.Next(23));
                ...
                if (Main.projectile[p].ai[1] == 19)
                {
                    for (int k = 0; k < 2; k++)
                    {
                        ...
                        int q = Projectile.NewProjectile(Projectile.GetSource_Death(), Projectile.position.X, Projectile.position.Y, perturbedSpeed.X, perturbedSpeed.Y, ModContent.ProjectileType<OreChunkM>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, 5, 19);
