    ...
    public class DeityRose : ModNPC
	{
        ...
        public override void AI()
        {
            ...
            else
            {
                ...
                {
                    float num729 = 8f;
                    Vector2 vector90 = new Vector2(NPC.position.X + NPC.width * 0.5f, NPC.position.Y + NPC.height * 0.5f);
                    float num730 = Main.player[NPC.target].position.X + Main.player[NPC.target].width * 0.5f - vector90.X + Main.rand.Next(-10, 11);
                    float num731 = Math.Abs(num730 * 0.2f);
                    float num732 = Main.player[NPC.target].position.Y + Main.player[NPC.target].height * 0.5f - vector90.Y + Main.rand.Next(-10, 11);
                    if (num732 > 0f)
                    {
                        num731 = 0f;
                    }
