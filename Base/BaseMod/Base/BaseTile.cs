    ...
    public class BaseTile
    {
            ...
            for (int x1 = leftX; x1 < rightX; x1++)
            {
                for (int y1 = leftY; y1 < rightY; y1++)
                {
                    ...
                    if (tile is { HasTile: true } && types.Contains(tile.TileType) && (!noSloped || tile.Slope == SlopeType.Solid) && (addTile == null || addTile(tile)) && (dist == -1 || Vector2.Distance(originalPos, new Vector2(x1, y1)) < dist))
                    {
                        ...
                        if (tile.TileType == TileID.Containers || TileObjectData.GetTileData(tile.TileType, 0) != null && (TileObjectData.GetTileData(tile.TileType, 0).Width > 1 || TileObjectData.GetTileData(tile.TileType, 0).Height > 1))
                        {
                            int x2 = x1; int y2 = y1;
        ...
        public static int LiquidCount(int x, int y, int distance = 25, int liquidType = 0)
        {
            ...
            for (int x1 = leftX; x1 < rightX; x1++)
            {
                ...
                {
                    Tile tile = Framing.GetTileSafely(x1, y1);
                    if (tile is { LiquidAmount: > 0 } && (liquidType == 0 ? tile.LiquidType == LiquidID.Water : liquidType == 1 ? tile.LiquidType == LiquidID.Lava : tile.LiquidType == LiquidID.Honey))
                    {
        ...
        public static bool NoLiquidTileInArea(int x, int y, int distance = 25)
        {
            int leftX = Math.Max(10, x - distance);
            int leftY = Math.Max(10, y - distance);
            int rightX = Math.Min(Main.maxTilesX - 10, x + distance);
