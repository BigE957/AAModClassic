    ...
    public class BaseAI
    {
		...
		*/
		...
		public static void AIFriendly(NPC npc, ref float[] ai, float moveInterval = 0.07f, float maxSpeed = 1f, bool critter = false, bool? seekHome = null, bool canTeleportHome = true, bool canFindHouse = true, bool canOpenDoors = true)
		{
			...
			{
				if (Main.netMode != NetmodeID.MultiplayerClient && !npc.homeless && !Main.tileDungeon[(int)Main.tile[npcTileX, npcTileY].TileType] && (npcTileX < npc.homeTileX - 35 || npcTileX > npc.homeTileX + 35))
				{
				...
				if (npc.closeDoor && ((npc.Center.X / 16f > npc.doorX + 2) || (npc.Center.X / 16f < npc.doorX - 2)))
				{
					...
					{
						npc.closeDoor = false;
						NetMessage.SendData(MessageID.ToggleDoorState, -1, -1, NetworkText.FromLiteral(""), 1, (float)npc.doorX, (float)npc.doorY, (float)npc.direction, 0);
					}
				...
				if (canOpenDoors && Main.tile[tileX2, tileY2 - 2].HasUnactuatedTile && (int)Main.tile[tileX2, tileY2 - 2].TileType == 10 && (Main.rand.Next(10) == 0 || seekHouse))
				{
					...
					if (WorldGen.OpenDoor(tileX2, tileY2 - 2, npc.direction))
					{
						...
						npc.doorX = tileX2;
						npc.doorY = tileY2 - 2;
						NetMessage.SendData(MessageID.ToggleDoorState, -1, -1, NetworkText.FromLiteral(""), 0, (float)tileX2, (float)(tileY2 - 2), (float)npc.direction, 0);
						npc.netUpdate = true;
						ai[1] += 80f;
        ...
        public static bool AttemptOpenDoor(NPC npc, ref float doorBeatCounter, ref float doorCounter, ref float tickUpdater, float ticksUntilBoredom, int doorBeatCounterMax = 10, int doorCounterMax = 60, int interactDoorStyle = 0)
        {
            ...
            if (hitTile)
            {
                ...
                if (Main.tile[tileX, tileY - 1].HasUnactuatedTile && Main.tile[tileX, tileY - 1].TileType == TileID.ClosedDoor)
                {
                    doorCounter += 1f;
                    tickUpdater = 0f;
                    if (doorCounter >= doorCounterMax)
                    {
                        ...
                        if (attemptOpenDoor && Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            {
                                NetMessage.SendData(MessageID.ToggleDoorState, -1, -1, NetworkText.FromLiteral(""), 0, (float)tileX, (float)tileY, (float)npc.direction, 0);
                            }
                        }
                    }
                    return true;
        ...
        public static void DamagePlayer(Player player, int dmgAmt, float knockback, int hitDirection, Entity damager, bool dmgVariation = true, bool hitThroughDefense = false, int critChance = 0, float critMult = 1f)
        {
            ...
            if (damager == null)
            {
                ...
                {
                    NetMessage.SendData(MessageID.HurtPlayer, -1, -1, PlayerDeathReason.LegacyDefault().GetDeathText(player.name), player.whoAmI, (float)hitDirection, (float)1, knockback, parsedDamage);
                }
            }else
            ...
            if (damager is Projectile)
            {
                ...
                if(p.friendly)
                {
                    ...
                    if (Main.netMode != NetmodeID.SinglePlayer)
                    {
                        NetMessage.SendData(MessageID.HurtPlayer, -1, -1, PlayerDeathReason.ByProjectile(p.owner, p.whoAmI).GetDeathText(player.name), player.whoAmI, (float)hitDirection, (float)1, knockback, parsedDamage);
                    }
            ...
            if (damager is NPC)
            {
                ...
                if (Main.netMode != NetmodeID.SinglePlayer)
                {
                    NetMessage.SendData(MessageID.HurtPlayer, -1, -1, PlayerDeathReason.ByNPC(npc.whoAmI).GetDeathText(player.name), player.whoAmI, (float)hitDirection, (float)1, knockback, parsedDamage);
                }
            }
        }
        ...
        public static void DamageNPC(NPC npc, int dmgAmt, float knockback, int hitDirection, Entity damager, bool dmgVariation = true, bool hitThroughDefense = false)
        {
            ...
            if (damager == null)
            {
                ...
                if (Main.netMode != NetmodeID.SinglePlayer)
                {
                    NetMessage.SendData(MessageID.DamageNPC, -1, -1, NetworkText.FromLiteral(""), npc.whoAmI, (float)1, knockback, (float)hitDirection, parsedDamage);
                }
            }else
            if (damager is Projectile)
            {
            ...
            if (damager is Player)
            {
                ...
                if (player.whoAmI == Main.myPlayer)
                {
                    ...
                    if (Main.netMode != NetmodeID.SinglePlayer)
                    {
                        NetMessage.SendData(MessageID.DamageNPC, -1, -1, NetworkText.FromLiteral(""), npc.whoAmI, 1, knockback, (float)hitDirection, parsedDamage);
                    }
