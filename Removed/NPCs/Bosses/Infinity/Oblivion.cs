...
namespace AAModClassic.Removed.NPCs.Bosses.Infinity
{
    public class Oblivion : ModNPC
    {
            ...
            else if (AAPlayer.ZeroKills >= 5)
            {
                ...
                if (OblivionSpeech == 300)
                {
                    ...
                    else if (rand == 6)
                    {
                        ...
                        else if (AAModRemoved.fargoLoaded && Main.rand.Next(MajorModCount()) == 0)
                        {
                        ...
                        else if (AAModRemoved.redemptionLoaded && Main.rand.Next(MajorModCount()) == 0)
                        {
                            Main.NewText("If you have such a hardon for killing robots, the Vlitch are a thing, you know.", color1);
                        }
                        else if (AAModRemoved.tremorLoaded && Main.rand.Next(MajorModCount()) == 0)
                        {
                            Main.NewText("Wait you're playing Tremor? HAHAHAHAHAHAH!", color1);
                        }
                        else if (AAModRemoved.sacredToolsLoaded && Main.rand.Next(MajorModCount()) == 0)
                        {
                            Main.NewText("Go bug the Lunarians or something. I'm sure they'll be more fun to fight than I am.", color1);
                        }
                        else if (AAModRemoved.grealmLoaded && Main.rand.Next(MajorModCount()) == 0)
                        {
        ...
        public int MajorModCount()
        {
            ...
            if (AAModRemoved.calamityLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.thoriumLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.spiritLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.fargoLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.redemptionLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.tremorLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.sacredToolsLoaded)
            {
                ModCount++;
            }
            if (AAModRemoved.grealmLoaded)
            {
                ModCount++;
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            if (glowTex == null)
            {
                glowTex = Mod.GetTexture("Removed/NPCs/Bosses/Infinity/Oblivion_Glow");
            }
            if (glitchTex == null)
            {
                glitchTex = Mod.GetTexture("Removed/NPCs/Bosses/Infinity/OblivionGlitch");
            }
