...
namespace AAModClassic.Utilities.AbstractsLikeDigitalCircus
{
    public abstract class FlailHoldout : ModProjectile
    {
        ...
        public virtual float DrawRotationOffset => 0;
        public virtual int LaunchTimeLimit => 15;  // How much time the projectile can go before retracting (speed and shootTimer will set the flail's range)
        ...
        public override void AI()
        {
            ...
            float myLaunchSpeed = LaunchSpeed * meleeSpeedMultiplier;
            float myUnusedRetractAcceleration = UnusedRetractAcceleration * meleeSpeedMultiplier;
            float myUnusedMaxRetractSpeed = UnusedMaxRetractSpeed * meleeSpeedMultiplier;
            float myRetractAcceleration = RetractAcceleration * meleeSpeedMultiplier;
            float myMaxRetractSpeed = MaxRetractSpeed * meleeSpeedMultiplier;
            float myForcedRetractAcceleration = ForcedRetractAcceleration * meleeSpeedMultiplier;
            float myMaxForcedRetractSpeed = MaxForcedRetractSpeed * meleeSpeedMultiplier;
            float launchRange = myLaunchSpeed * LaunchTimeLimit;
                ...
                case AIState.Spinning:
                    {
                        ...
                        if (Projectile.owner == Main.myPlayer)
                        {
                            ...
                            if (!player.channel) // If the player releases then change to moving forward mode
                            {
                                ...
                                StateTimer = 0f;
                                Projectile.velocity = unitVectorTowardsMouse * myLaunchSpeed + player.velocity;
                ...
                case AIState.Retracting:
                    {
                        ...
                        if (Projectile.Distance(mountedCenter) <= myMaxRetractSpeed)
                        {
                ...
                case AIState.UnusedState:
                    {
                        ...
                        if (currentChainLength > UnusedChainLength)
                        {
                            ...
                            if (currentChainLength >= launchRange)
                            {
                                ...
                                Projectile.velocity = Projectile.velocity.MoveTowards(Projectile.DirectionTo(mountedCenter).SafeNormalize(Vector2.Zero) * myUnusedMaxRetractSpeed, myUnusedMaxRetractSpeed);
                            }
                            Projectile.velocity *= 0.98f;
                            Projectile.velocity = Projectile.velocity.MoveTowards(Projectile.DirectionTo(mountedCenter).SafeNormalize(Vector2.Zero) * myUnusedMaxRetractSpeed, myUnusedRetractAcceleration);
                        }
                        else
                        {
                ...
                case AIState.ForcedRetracting:
                    {
                        ...
                        Vector2 unitVectorTowardsPlayer = Projectile.DirectionTo(mountedCenter).SafeNormalize(Vector2.Zero);
                        if (Projectile.Distance(mountedCenter) <= myMaxForcedRetractSpeed)
                        {
                            Projectile.Kill(); // Kill the projectile once it is close enough to the player
                            return;
                        }
                        Projectile.velocity *= 0.98f;
                        Projectile.velocity = Projectile.velocity.MoveTowards(unitVectorTowardsPlayer * myMaxForcedRetractSpeed, myForcedRetractAcceleration);
