        ...
        public override void RandomUpdate(int i, int j, int type)
		{
            if (Main.expertMode)
            {
                if(DevWorld.CCBoxSetOK)
                {
                    ...
                    {
                        bool canplace = (type == ModContent.TileType<MireGrass>() || type == ModContent.TileType<Depthstone>()) && (Main.tile[i + 1, j - 1].TileType == ModContent.TileType<MireGrass>() || type == ModContent.TileType<Depthstone>()) && !Main.tile[i, j - 1].HasTile && !Main.tile[i + 1, j - 1].HasTile && j > Main.worldSurface + 200;
                        if(canplace)
                        {
                            WorldGen.PlaceTile(i, j - 1, ModContent.TileType<CCMireBox>(), true, false);
                ...
                if(DevWorld.InvokerBookSetOK)
                {
                    if(NPC.downedPlantBoss)
                    {
                        ...
                        if(canplace)
                        {
                            WorldGen.PlaceTile(i, j - 1, ModContent.TileType<InvokerBookTile>(), true, false);
