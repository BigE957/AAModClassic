...
Grants 1 soul essence on direct hit"); */			
		...
		public override void SetDefaults()
		{
			...
			Item.autoReuse = true;
            Item.shoot = Mod.Find<ModProjectile>("DragonsClaw").Type;
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			for (int k = 0; k < 2; k++)
			{
				Projectile.NewProjectile(source, player.Center.X, player.Center.Y, 0f, 0f, Mod.Find<ModProjectile>("DragonsClawEffect").Type, damage, knockback, player.whoAmI, k, 0f);
			}
			return true;
		}
