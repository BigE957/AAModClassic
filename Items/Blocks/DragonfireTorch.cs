...
namespace AAModClassic.Items.Blocks
{
	public class DragonfireTorch : BaseAAItem
    {
		...
		public override void SetDefaults()
		{
			...
			Item.createTile = ModContent.TileType<DragonfireTorch>();
			Item.flame = true;
		...
		public override void HoldItem(Player player)
		{
			...
			{
				Dust.NewDust(new Vector2(player.itemLocation.X + 16f * player.direction, player.itemLocation.Y - 14f * player.gravDir), 4, 4, ModContent.DustType<DragonflameDust>());
			}
			Vector2 position = player.RotatedRelativePoint(new Vector2(player.itemLocation.X + 12f * player.direction + player.velocity.X, player.itemLocation.Y - 14f + player.velocity.Y), true);
			Lighting.AddLight(position, AAColor.DragonFire.R / 255, AAColor.DragonFire.G / 255, AAColor.DragonFire.B / 255);
		}
