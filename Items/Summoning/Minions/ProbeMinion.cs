...
namespace AAModClassic.Items.Summoning.Minions
{
    public class ProbeMinion : ModProjectile
	{
        ...
        {
            bool flag64 = Projectile.type == Mod.Find<ModProjectile>("ProbeMinion").Type;
            Player player = Main.player[Projectile.owner];
            AAPlayer modPlayer = player.GetModPlayer<AAPlayer>();
            player.AddBuff(Mod.Find<ModBuff>("ProbeMinion").Type, 3600);
            if (flag64)
            {
                if (player.dead)
                {
            ...
            {
                bool flag23 = Main.projectile[num638].type == Mod.Find<ModProjectile>("ProbeMinion").Type;
            ...
            if (Projectile.ai[0] == 0f)
            {
                ...
                if (flag25 && Projectile.ai[1] == 0f)
                {
                    ...
                    if (Main.myPlayer == Projectile.owner && Collision.CanHitLine(Projectile.position, Projectile.width, Projectile.height, vector46, 0, 0))
                    {
                        ...
                        int num659 = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, value19.X, value19.Y, num658, (int)(Projectile.damage * 0.8f), 0f, Main.myPlayer, 0f, 0f);
