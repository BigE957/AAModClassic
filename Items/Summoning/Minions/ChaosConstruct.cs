...
namespace AAModClassic.Items.Summoning.Minions
{
    public class ChaosConstruct : ModProjectile
    {
		...
		{
			bool flag64 = Projectile.type == ModContent.ProjectileType<ChaosConstruct>();
			Player player = Main.player[Projectile.owner];
			AAPlayer modPlayer = player.GetModPlayer<AAPlayer>();
			player.AddBuff(ModContent.BuffType<ChaosConstruct_Buff>(), 3600);
			if (flag64)
			{
				if (player.dead)
				{
			...
			{
				bool flag23 = Main.projectile[num638].type == ModContent.ProjectileType<ChaosConstruct>();
