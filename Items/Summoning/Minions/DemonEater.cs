...
namespace AAModClassic.Items.Summoning.Minions
{
    public class DemonEater : ModProjectile
    {
        ...
        {
            bool flag64 = Projectile.type == ModContent.ProjectileType<DemonEater>();
            Player player = Main.player[Projectile.owner];
            AAPlayer modPlayer = player.GetModPlayer<AAPlayer>();
            player.AddBuff(ModContent.BuffType<EaterMinion_Buff>(), 3600);
            if (flag64)
            {
                if (player.dead)
                {
			...
			{
				bool flag23 = Main.projectile[num638].type == ModContent.ProjectileType<DemonEater>();
