...
public class KeepCandle_Tile : ModTile
{
	...
	public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
	{
		...
		for (int k = 0; k < 7; k++)
		{
			...
			Main.spriteBatch.Draw(ModContent.Request<Texture2D>(ModContent.GetInstance<TerraCandle_Tile>().Texture + "_Flame").Value, new Vector2((float)(i * 16 - (int)Main.screenPosition.X + num4) - ((float)num - 16f) / 2f + num5, (float)(j * 16 - (int)Main.screenPosition.Y + num2) + num6) + zero, (Rectangle?)new Rectangle(frameX, frameY, num, num3), val, 0f, default(Vector2), 1f, (SpriteEffects)0, 0f);
		}
	}
}
